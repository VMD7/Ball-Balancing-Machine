...
def get_vision_sensor_image():
	...
	##############	ADD YOUR CODE HERE	##############
	_, handle = sim.simxGetObjectHandle(client_id,'vision_sensor_1', sim.simx_opmode_blocking)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	## Converting Image list to numpy array with data as uint 8 ##
	image_arr = np.array(vision_sensor_image)
	image_arr = image_arr.astype(np.uint8)

	## Resizing 1-D array to 3-D array ##
	image1 = np.reshape(image_arr,(image_resolution[0],image_resolution[1],3))

	## Changing color of an Image from BGR to RGB. ##
	converted_img = cv2.cvtColor(image1,cv2.COLOR_BGR2RGB)

	## Flipping the Image about X axis ##
	transformed_image = cv2.flip(converted_img, 0)
